<div class="grid" x-data="{ show: false }">
    <div class="flex justify-between text-sm items-center w-full py-2 px-3 bg-gray-300 text-grayd">
        <a href="{{ route('record', ['account' => $account->nickname, 'slug' => $publication->slug]) }}"
            class="hover:text-secondary">
            {{ \Illuminate\Support\Str::limit(\Illuminate\Support\Str::words($publication->subtitle, 4, ''), 40, '...') }}
        ...
        <div class="flex items-center gap-2">
            @livewire('likes.like', ['likeable' => $publication, 'user' => auth()->user()])
        </div>
    </div>
    ...
    <a href="{{ route('record', ['account' => $account->nickname, 'slug' => $publication->slug]) }}"
        ...
        @if ($newProject)
            <div class="hover:opacity-30 transition-all">
                <img class="object-cover aspect-[2/1] w-full" src="{{ $this->beforeImage }}" alt="imagen">
            </div>
        @else
