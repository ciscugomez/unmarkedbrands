                ...
                <div wire:sortable.item="{{ $key }}" wire:key="sort-item-{{ $key }}"
                    ...
                    <div class="flex justify-start items-center gap-5">
                        ...
                        <div class="flex gap-2 p-4">
                            {{-- Miramos si hay archivos nuevos subidos --}}
                            {{-- Si no hay archivos nuevos para subir, mostremos la preview de archivos existentes --}}
                            {{-- Si hay archivos nuevos, me motraran la preview de esas --}}
                            @if (count($item['new_urls']) == 0 && !isset($item['is_new']))
                                @foreach ($item['value'] as $image)
                                    <img src="{{ \Illuminate\Support\Facades\Storage::disk('s3')->temporaryUrl('projects/' . $image, now()->addMinutes(5)) }}"
                                        alt="imagen" class="h-11 aspect-square object-cover">
                                @endforeach
                            @else
                                @foreach ($item['new_urls'] as $image)
